...
**API:** `GET /api/analysis/fundamental?ticker=AAPL&include_rag=true` returns filing **`rag_evidence`** when SEC ingest is configured; add **`include_llm=true`** (and `provider` / `model`) so the AI summary can use those excerpts. See **Fundamental analysis with SEC RAG** under API examples below.
...
| GET | `/api/analysis/fundamental` | Fundamental analysis (Bearer auth). Query: `ticker`, optional `include_llm`, `include_rag`, `provider`, `model` |
...
curl -s "http://127.0.0.1:8000/api/analysis/fundamental?ticker=AAPL&include_rag=true&include_llm=true&provider=gemini&model=gemini-3-flash-preview" \
  -H "Authorization: Bearer $TOKEN" | python3 -m json.tool | head -80
```
