...
def get_fundamental_analysis(
    ...
    if include_llm:
        ...
        elif isinstance(ai_summary, str) and ai_summary.strip() and _fundamental_summary_needs_repair(ai_summary, data):
            # Critic: if summary contains significant numbers not in deterministic JSON, rewrite once.
            repair_prompt = build_fundamental_repair_prompt(
                report=data,
                previous_text=ai_summary,
                problem="numeric_claim_not_in_deterministic_report",
            )
            repair = generate_text(repair_prompt, provider=provider, model=model)
            if not repair.get("error") and repair.get("text"):
                ai_summary = str(repair.get("text") or "").strip()
                ai_model = repair.get("model") or ai_model
            else:
                # Keep original summary but flag best-effort nature.
